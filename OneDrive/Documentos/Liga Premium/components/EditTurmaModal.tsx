...
import { updateTurma, deleteTurma } from '@/lib/firebase/turmas';
...
export default function EditTurmaModal({ isOpen, onClose, onSuccess, turma }: EditTurmaModalProps) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await updateTurma(turma.id, name.trim());
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
      <div className="bg-gray-800 rounded-lg p-6 max-w-md w-full mx-4 border border-gray-700">
        <h3 className="text-xl font-bold mb-4 text-gray-100">Editar Turma</h3>
        <form onSubmit={handleSave} className="space-y-4">
          ...
          >
            ...
            <span>{showDeleteConfirm ? 'Confirmar Remoção' : 'Remover Turma'}</span>
          </button>
        </form>
      </div>
    </div>
